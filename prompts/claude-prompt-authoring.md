Use `orchestration-bridge:prompt-authoring` to draft or repair a prompt for a selected receiver.

Goal: produce a compact, outcome-first prompt. The authoring profile is fable-5.1 or opus-5; select the receiver's runtime/profile/kind independently. Keep only task-critical context and reference reusable workflow.
