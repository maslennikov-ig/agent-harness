Target: Claude Code design owner using the installed `/impeccable` marketplace skill.
...
- For visually designed HTML-to-PDF, use Impeccable for visual craft and the PDF workflow for rendering, pagination, fonts, layout fidelity, and visual inspection. Plain PDF extraction, merge, OCR, or administration stays with the PDF workflow.
- Route medium/complex design execution through `orchestration-bridge:orchestrator-stage`: delegate a stream only for a concrete parallel, context-isolation, specialist, or write-isolation benefit, otherwise execute directly; unavailable subagents never block. The root retains coordination, shared decisions, integration, final acceptance, and delivery; the stage skill owns safe parallel fan-out.
