...
if "requires_explicit_user_spawn_request" in delegation:
    raise SystemExit(
        "delegation.requires_explicit_user_spawn_request is stale; delegation is preauthorized by benefit"
    )
...
if launcher == "codex_subagents":
    ...
    if delegation.get("sequential_requires_reason") is not False:
        raise SystemExit(
            "delegation.sequential_requires_reason must be false"
        )
...
if launcher == "codex_subagents":
    ...
    stale_present = sorted(stale_model_keys.intersection(model_policy))
    if stale_present:
        raise SystemExit(
            "subagent_model_policy contains stale rigid routing keys: "
            + ", ".join(stale_present)
        )
