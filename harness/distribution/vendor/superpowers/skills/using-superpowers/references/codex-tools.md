...
Apply the active delegation policy before spawning. Root performs only simple,
quick substantive work; medium and complex substantive execution is delegated.
Routine task completion does not wait for a per-task reviewer. Worker return
also does not create a per-task acceptance gate.
